...
class MouseListener(fife.IMouseListener):
    ...
    def mouseDragged(self, event):
        ...
        if event.getButton() == fife.MouseEvent.LEFT and hasattr(self, "select_begin"):
            do_multi = (
                (self.select_begin[0] - event.getX()) ** 2
                + (self.select_begin[1] - event.getY()) ** 2
            ) >= 10  # from 3px (3*3 + 1)
            genericrenderer = fife.GenericRenderer.getInstance(self._test._camera)
            genericrenderer.removeAll("selection")
            if do_multi:
                ...
                b = fife.Point(
                    max(self.select_begin[0], event.getX()),
                    min(self.select_begin[1], event.getY()),
                )
                c = fife.Point(
                    max(self.select_begin[0], event.getX()),
                ...
                genericrenderer.addLine(
                    "selection", fife.RendererNode(a), fife.RendererNode(b), 200, 200, 200
                )
                genericrenderer.addLine(
                    "selection", fife.RendererNode(b), fife.RendererNode(c), 200, 200, 200
                )
                genericrenderer.addLine(
                    "selection", fife.RendererNode(d), fife.RendererNode(c), 200, 200, 200
                )
                genericrenderer.addLine(
                    "selection", fife.RendererNode(a), fife.RendererNode(d), 200, 200, 200
                )
