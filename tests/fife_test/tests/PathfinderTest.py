...
class PathfinderTest(test.Test):
    ...
    def getHelp(self):
        return open('data/help/PathfinderTest.txt', 'r').read()
    ...
    def getLocationAt(self, screenpoint):
        ...
        for l in reversed(layers):
            ...
            if len(instances) > 0:
                if l.getId() == "peak_item_layer" or l.getId(
                ) == "peak_ground_layer":
                    loc.setLayer(self._peakactorlayer)
                    location = loc
                    break
                if l.getId() == "top_item_layer" or l.getId(
                ) == "top_ground_layer":
                    loc.setLayer(self._topactorlayer)
                    location = loc
                    break
                if l.getId() == "item_layer" or l.getId(
                ) == "ground_layer" or l.getId() == "water_layer":
