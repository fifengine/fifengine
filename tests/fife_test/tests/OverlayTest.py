...
class OverlayTest(test.Test):
    ...
    def run(self):
        ...
        self._font = pychan.internal.get_manager().createFont(
            "data/fonts/rpgfont.png")
    ...
    def loadMap(self, filename):
        ...
        self._instance_renderer = fife.InstanceRenderer.getInstance(
            self._camera)
    ...
    def createStaticColorOverlays(self):
        ...
        overlay.changeColor(
            fife.Color(255, 255, 255), fife.Color(0, 0, 255, 128))
        for n in (1, 5, 9):
    ...
    def createColorOverlay(self):
        ...
        for rotation in range(0, 360, 45):
            anim = self._animationmanager.create("skel_stand_overlay" +
                                                 str(rotation))
            for frame in range(4):
                resource = ("overlay_stand_" + str(rotation) + "_" + str(frame)
                            + ".png")
            ...
            # skel 1
            overlay.changeColor(
                fife.Color(255, 0, 0), fife.Color(255, 0, 0, 80))
            overlay.changeColor(
                fife.Color(0, 0, 255), fife.Color(0, 0, 255, 80))
            self._skel1.addColorOverlay("stand", rotation, overlay)
            # skel 2
            overlay.changeColor(
                fife.Color(255, 0, 0), fife.Color(0, 255, 0, 128))
            overlay.changeColor(
                fife.Color(0, 0, 255), fife.Color(0, 255, 0, 128))
    ...
    def createColoringAndOutlines(self):
        instances = [
            self._actorlayer.getInstance("toilett%s" % n)
            for n in range(4, 12)
        ]
