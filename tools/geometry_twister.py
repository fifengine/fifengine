...
class Shape:
    def __init__(self, center_pt, sidecount):
        ...
        self.angles = [
            startangle + 2 * math.pi * (s / float(sidecount))
            for s in range(sidecount - 1)
        ]
        shape_pts = [(0, 0, 0)]
        maxy, miny = 0, 0
    ...
    def transform_pts(self, pts):
        ...
        for x, y, z in tilted_pts:
            zoomed_pts.append(
                (
                    int(round(self.center_pt[0] + self.zoomval * x)),
...
class Gui:
    ...
    def __init__(self):
        ...
        self.canvas.create_window(
            285,
            280,
            window=TK.Frame(self.canvas, relief=TK.GROOVE, borderwidth=2),
            anchor=TK.CENTER,
        )
    ...
    def draw_shape(self, shapepts, boundpts=None, fillshape=True):
        ...
        if not fillshape:
            fillcolor = ""
        self.canvas.create_polygon(
            fill=fillcolor, outline="black", tag="shape", *self.flatten_pts(shapepts)
        )
    ...
    def update_texts(self, size, transform):
        ...
        self.canvas.create_text(
            10, 370, text="Left button + drag = tilt/rotate", anchor=TK.NW
        )
    ...
    def update_view(self):
        ...
        transform = (
            reflections[0][0][0] - shapepts[0][0],
            -(reflections[0][0][1] - shapepts[0][1]),
        )
