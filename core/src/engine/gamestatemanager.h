...
namespace FIFE {
	...
	/** A simple state-machine (SM) for objects encapsulating the game-specific code.
	 *
	 ...
	 *
	 * @note The function activate does check whether a state is already active;
	 * It will currently issue a warning and @b not activate it again.
	 *
   ...
   */
	class GameStateManager : public DynamicSingleton<GameStateManager> {
		public:
			...
			/** Query if a certain state is known (registered) to the SM.
			 *
			 * @param name The name of the state.
			 * @return True if the state exists.
			 * @note This doesn't mean @b active; just that it exists.
			 */
			bool gameStateExists(const std::string& name) const;
			
			/** Access to the existing states by name.
			 * @param name The name of the state.
			 * @return Pointer to requested state (if it exists).
			 *
			 * @throws NotFound exception on failure.
			 */
