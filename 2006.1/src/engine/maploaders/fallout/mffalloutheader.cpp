...
namespace FIFE {
	...
	MFFalloutHeader::MFFalloutHeader(RawDataPtr data) : m_mapname(), m_version(0), m_defaultPlayerPos(0), m_defaultElevation(0),
	m_defaultOrientation(0), m_num_localvars(0), m_scriptid(0), m_elevations(0), m_num_globalvars(0), m_mapID(0), m_timestamp(0) {
		loadHeader(data);
	}


	MFFalloutHeader::~MFFalloutHeader() {}

	void MFFalloutHeader::loadHeader(RawDataPtr data) {
		data->setIndex(0);
		...
		Log("mlfallout") << "Elevationflags: " /*<< std::hex*/ << m_elevations /*<< std::dec*/ << " ->" << str_elevations;
		Log("mlfallout") << "Globalvars: " << m_num_globalvars;
		Log("mlfallout") << "MapID: " << m_mapID;
		Log("mlfallout") << "Timestamp: " << m_timestamp;
	}

	void MFFalloutHeader::checkSanity() {
		if (m_version != 19 && m_version != 20) {
			Log("MFFalloutHeader") << "invalid version: " << m_version;
	...
	const std::string& MFFalloutHeader::getMapName() const {
		return m_mapname;
	}

	unsigned int MFFalloutHeader::getNumElevations() const {
	...
	uint32_t MFFalloutHeader::getVersion() const {
		return m_version;
	}

	uint32_t MFFalloutHeader::getElevations() const {
		return m_elevations;
	}

	uint32_t MFFalloutHeader::getNumLocalVars() const {
		return m_num_localvars;
	}

	uint32_t MFFalloutHeader::getNumGlobalVars() const {
		return m_num_globalvars;
	}

	bool MFFalloutHeader::hasElevation(unsigned int num) const {
		if (num >= 3)
			return false;
