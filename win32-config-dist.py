...
def addExtras(context):
	path = os.environ['_']
	context.env.Append(CPPPATH = [path + '\\OpenAL\\Include', path + '\\SDL_image', path + '\\zlib', path + '\\lua\\include', path + '\\guichan', path + '\\boost', path + '\\libvorbis', path, path + '\\sdl_ttf\\include', path + '\\sdl\\include'])
	context.env.Append(LIBPATH = [path + '\\lib_diverse'])
	
	context.env.Append(LIBS = ['fife', 'mingw32', 'zlib', 'SDL_image', 'lua50', 'guichan', 'guichan_sdl', 'vorbis', 'ogg', 'vorbisfile', 'SDLmain', 'SDL', 'OpenAL32', 'SDL_ttf', 'boost_filesystem', 'boost_regex'])
