...
gui.actions.on_stresstest = function()
  ...
  -- Read maps
  local file = io.open("content/etc/maps_fo2.txt")
  local i = 0
  lchooser.toload = {}
...
function create_level_listmodel(...)
	local lm = ListModel()
...
function create_window(title,sx,sy, font)
  ...
  -- hack-around-bug? do _not_ set the size of the window, it
  -- will resize to the container
  -- activate & check the last drop-down entry to see the
  -- change; not enough space for the widget
...
function main_menu()
  ...
  sx = 330
  sy = 400
  level_list = create_level_listmodel("content/etc/maps_fo2.txt","content/etc/maps_custom.txt")
  ...
  inner = create_window("Choose a FO2 Map",sx,sy, small_font)
