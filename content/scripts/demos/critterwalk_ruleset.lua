...
TestCritter = class(GObject,function(self,params)
	local CritterBaseString
	GObject.init(self,params)
	self.animations = {}
	
	self.visual = params.visual
	CritterBaseString = string.sub(self.visual, 0, -7)

	self.animations.idle = CritterBaseString .. "aa.frm"
	self.animations.walk = CritterBaseString .. "ab.frm"
	self.animations.kick = CritterBaseString .. "ar.frm"
...
function TestCritter:onWake(ticks)
	FIFE_Sleep(self.ref, self:stateFunction(ticks) )
end
...
-- 		" with visual ",params.visual)
	...
	if self.active_elevation == params.location.elevation then
	end
end
...
function Ruleset:activateElevation(elevation)
	...
	for index = 1,self.map_elevation[elevation].params.n do
		local params
		params = self.map_elevation[elevation].params[index]
