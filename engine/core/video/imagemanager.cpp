...
namespace FIFE {
	...
	void ImageManager::createAtlas(const std::string& filename) {
		// TODO - use boost filesystem and RawData jstu like in /loaders/map
		TiXmlDocument doc(filename);
		bool loaded = doc.LoadFile();

		if (!loaded) {
			fprintf(stderr, "Failed to load atlas file %s\n", filename.c_str());
			return;
		}

		TiXmlElement* root = doc.RootElement();
		if(root) {
			for (TiXmlElement* atlasElem = root->FirstChildElement("atlas");
				 atlasElem != 0; atlasElem = atlasElem->NextSiblingElement("atlas")) {

				const std::string* atlasName = atlasElem->Attribute(std::string("name"));
				if(atlasName) {
					AtlasPtr newAtlas = AtlasPtr(new Atlas);

					int atlasWidth, atlasHeight;
					atlasElem->QueryValueAttribute("width", &atlasWidth);
					atlasElem->QueryValueAttribute("height", &atlasHeight);

					for (TiXmlElement* imageElem = atlasElem->FirstChildElement("image");
						...
						newAtlas->m_subimages.insert(std::make_pair(*subimageName, region));
					}
					newAtlas->m_image = load(*atlasName);
 					newAtlas->m_image->forceLoadInternal();

					for (Atlas::SubimageMap::iterator it = newAtlas->m_subimages.begin(); it != newAtlas->m_subimages.end(); ++it) {
						ImagePtr subImage = create(it->first);
						subImage->useSharedImage(newAtlas->m_image, it->second, atlasWidth, atlasHeight);
					}
