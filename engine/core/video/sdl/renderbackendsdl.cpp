...
namespace FIFE {
	...
	void RenderBackendSDL::addImageToArray(uint32_t& id, Rect& rec, float const* st, uint8_t& alpha) {
	}

	void RenderBackendSDL::addImageToArray2T(uint32_t& id, Rect& rec, float const* st, uint8_t& alpha, uint8_t const* rgb) {
	}
