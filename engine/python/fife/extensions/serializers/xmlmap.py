...
class XMLMapLoader(object):
    ...
    def parse_map(self, mapelt):
        ...
        if not format == FORMAT:
            self._err(''.join([
                'This file has format ', format,
                ' but this loader has format ', FORMAT
            ]))
        ...
        try:
            ...
            print(e.getMessage())
            print(''.join([
                'File: ', self.source, '. The map ',
                str(_id), ' already exists! Ignoring map definition.'
            ]))
            return map
    ...
    def parse_layers(self, mapelt, map):
        ...
        for layer in mapelt.findall('layer'):
            ...
            if not grid_type:
                self._err(''.join(
                    ['Layer ',
                     str(_id), ' has no grid_type attribute.']))
            ...
            if not cellgrid:
                self._err('<layer> declared with invalid cellgrid type. (%s)' %
                          grid_type)
            ...
            if self.callback is not None:
                i += 1
                self.callback(self.msg['layer'] % str(_id),
    ...
    def parse_instances(self, layerelt, layer):
        ...
        for instance in instances:
            ...
            for attr in ('o', 'object', 'obj'):
                ...
                if objectID: break
            if not objectID:
                self._err('<instance> %s does not specify an object attribute.'
                          % str(objectID))
    ...
    def parse_cameras(self, mapelt, map):
        ...
        for camera in mapelt.findall('camera'):
            ...
            cam.setCellImageDimensions(
                int(ref_cell_width), int(ref_cell_height))
