...
class XMLMapSaver(object):
    def __init__(self, filepath, engine, map, importList, state=0,
                 datastate=0):
            ...
            for instance in layer.getInstances():
                ...
                if not (file in imports):
                    if not self.have_superdir(file, importList):
                        imports.append(file)
                        self.write_import(
                            root_subfile(map.getFilename(), file))
    ...
    def write_instances(self, layer):
        ...
        for inst in layer.getInstances():
            ...
            if inst.isOverrideBlocking():
                attr_vals[(None, 'override_blocking'
                           )] = str(int(inst.isOverrideBlocking()))
        ...
        for cam in cameras:
            ...
            for group in groups:
                ...
                for info in infos:
                    ...
                    attr_vals[(None, 'instance'
                               )] = str(info.getNode().getInstance().getId())
                    ...
                    if type == 'simple':
                        ...
                        if info.getSubdivisions() is not 32:
                            attr_vals[(
                                None,
                                'subdivisions')] = str(info.getSubdivisions())
        ...
        for cam in cameralist:
            if cam.getLocationRef().getMap().getId() == map.getId():
                ...
                if colors[0] < 1.0 or colors[1] < 1.0 or colors[2] < 1.0:
                    ...
                    attr_vals[(None, 'light_color')] = '%f,%f,%f' % (colors[0],
                                                                     colors[1],
                                                                     colors[2])
                attrs = AttributesNSImpl(attr_vals, attr_names)
