    ...
    def save(self, object):
        ...
        if not file:
            raise SerializerError(
                "Object cannot be saved, no file found %s" % object)
            return result
...
class XMLObjectLoader(object):
    ...
    def parse_object(self, object):
        ...
        if parent:
            ...
            if len(query) == 0:
                raise NotFound(
                    'No objects found with identifier %s.' % str(parent))
            elif len(query) > 1:
    ...
    def parse_actions(self, objelt, object):
        ...
        for action in objelt.findall('action'):
            ...
            if not id:
                raise InvalidFormat(
                    '<action> declared without an id attribute.')
    ...
    def parse_animations(self, actelt, action):
        ...
        for anim in actelt.findall('animation'):
            ...
            if not source:
                raise InvalidFormat(
                    'Animation declared with no source location.')
            ...
            action.get2dGfxVisual().addAnimation(
                int(anim.get('direction', 0)), animation)
