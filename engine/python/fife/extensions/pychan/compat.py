...
def _munge_engine_hook(engine):
    engine.translate_mouse_event = getattr(engine, 'translate_mouse_event',
                                           lambda x: x)
    engine.translate_key_event = getattr(engine, 'translate_key_event',
                                         lambda x: x)
    ...
    guimanager.setDefaultFont(engine.getSettings().getDefaultFontPath(),
                              engine.getSettings().getDefaultFontSize(),
                              engine.getSettings().getDefaultFontGlyphs())

    guimanager.init(engine.getRenderBackend().getName(),
...
class _multilistener(fifechan.ActionListener, fifechan.MouseListener,
                     fifechan.KeyListener):
    def __init__(self):
