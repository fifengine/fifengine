...
class Tab(Container):
    ...
    def clone(self, prefix):
        tabClone = Tab(
            ...
            self.background_image, self.opaque)
...
class TabbedArea(Container):
    ...
    def __init__(self,
        ...
        if select_tab_index is not None:
            self.select_tab_index = select_tab_index
        elif select_tab is not None:
    ...
    def clone(self, prefix):
        tabbedareaClone = TabbedArea(
            ...
            self.outline_color, self.border_size, self.outline_size,
            self.position_technique, self.is_focusable, self.comment,
            self.background_image, self.opaque, select_tab_index)
    ...
    def removeChild(self, widget):
        if not widget in self.children:
            raise RuntimeError("%s does not have %s as direct tab widget." %
                               (str(self), str(widget)))
    ...
    def getTabDefinition(self, widget):
        ...
        if name in self.tab_definitions:
            ...
            raise RuntimeError("%s does not have %s in the tab definitions." %
                               (str(self), str(name)))
