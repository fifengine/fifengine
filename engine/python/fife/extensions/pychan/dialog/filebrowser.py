    ...
    def showBrowser(self):
        ...
        if self.savefile:
            ...
            self._widget.findChild(
                name="fileColumn").addChild(self._file_entry)
    ...
    def setDirectory(self, path):
        ...
        try:
            self.dir_list = sorted(
                decodeList(self.dir_list), key=lambda s: s.lower())
            self.dir_list.insert(0, "..")
            self.file_list = sorted(
                decodeList(self.file_list), key=lambda s: s.lower())
        except:
            self.path = path_copy
            self.dir_list = list(dir_list_copy)
            self.file_list = list(file_list_copy)
            print(
                "WARNING: Tried to browse to directory that is not accessible!"
            )
