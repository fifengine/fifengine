...
namespace FIFE {
  VideoGameState::VideoGameState() : 
    GameState("MoviePlayer"), filename("test.avi") {
    sdl_ov = 0;
  }
  ...
  void VideoGameState::SDL_Video_Overlay::showFrame() {
    ...
    for (int y=0; y<codecCtx->height; y++) {
      memcpy(yuv_overlay->pixels[0]+yuv_overlay->pitches[0]*(y+offset_h) + offset_y, frame->data[0]+frame->linesize[0]*y,
        codecCtx->width);
    }
    // why /4? seems to work
    offset_h = (yuv_overlay->h - codecCtx->height)/4;
    for (int y=0; y<codecCtx->height/2; y++) {
      memcpy(yuv_overlay->pixels[1]+yuv_overlay->pitches[1]*(y+offset_h) + offset_u, frame->data[1]+frame->linesize[1]*y,
        codecCtx->width/2);
      memcpy(yuv_overlay->pixels[2]+yuv_overlay->pitches[2]*(y+offset_h) + offset_v, frame->data[2]+frame->linesize[2]*y,
        codecCtx->width/2);
