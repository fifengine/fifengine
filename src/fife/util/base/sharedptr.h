...
namespace FIFE
{
    ...
    class SharedPtr
    {
        public:
             ...
             */
            template <typename U>
            explicit SharedPtr(U* ptr) : m_ptr(ptr), m_refCount(ptr ? new uint32_t(1) : 0)
            {
            }
             ...
             */
            SharedPtr(SharedPtr const & rhs) : m_ptr(rhs.m_ptr), m_refCount(rhs.m_refCount)
            {
             ...
             */
            template <typename U>
            explicit SharedPtr(SharedPtr<U> const & rhs) : m_ptr(rhs.get()), m_refCount(rhs.useCountPtr())
            {
                incRefCount();
            }
