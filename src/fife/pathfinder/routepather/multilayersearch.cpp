...
namespace FIFE
{
    ...
    void MultiLayerSearch::createSearchFrontier(int32_t startInt, CellCache* cache)
    {
        ...
        int32_t const max_index = cache->getMaxIndex();
    ...
    void MultiLayerSearch::updateSearch()
    {
        ...
        ModelCoordinate const destCoord = m_currentCache->convertIntToCoord(m_lastDestCoordInt);
        ModelCoordinate const nextCoord = m_currentCache->convertIntToCoord(m_next);
    ...
    void MultiLayerSearch::calcPathStep()
    {
        ...
        int32_t const end = m_lastStartCoordInt;
    ...
    void MultiLayerSearch::searchBetweenTargetsNeighbor()
    {
        ...
        if (!cells.empty()) {
            ...
            for (auto& it : cells) {
                ...
                double const temp_distance    = temp.getLayerDistanceTo(m_from) + loc2.getLayerDistanceTo(m_to);
                double const current_distance = loc.getLayerDistanceTo(m_from) + loc1.getLayerDistanceTo(m_to);
    ...
    void MultiLayerSearch::searchBetweenTargetsMap()
    {
        ...
        auto allLayers = m_from.getLayer()->getMap()->getLayers();
        ...
        // start zone
        int32_t const startZone = zoneDistanceMap.find(m_startZone)->second;
        // target zone
        int32_t const targetZone = zoneDistanceMap.find(m_endZone)->second;
        ...
        while (!found) {
            ...
            spt.at(nextIndex)           = sf.at(nextIndex);
            ...
            for (; cell_it != transCells.end(); ++cell_it) {
                ...
                // next zone as int
                int32_t const nextInt = zoneDistanceMap.find(transCelle->getZone())->second;
                if (nextInt == next) {
                    continue;
                }
                // iterator distance as cost
                double const cost = costs.at(nextIndex) + static_cast<double>(std::abs(nextInt - startZone));
                std::size_t const nextIntIndex = toIndex(nextInt);
                if (sf.at(nextIntIndex) == -1) {
                    sortedfrontier.pushElement(PriorityQueue<int32_t, double>::value_type(nextInt, cost));
                    costs.at(nextIntIndex) = cost;
                    sf.at(nextIntIndex)    = next;
                } else if (cost < costs.at(nextIntIndex) && spt.at(nextIntIndex) == -1) {
        ...
        if (found) {
            ...
            while (current != end) {
                std::size_t const currentIndex = toIndex(current);
                if (spt.at(currentIndex) < 0) {
                    return;
                }
                current  = spt.at(currentIndex);
            ...
            for (auto lit = betweenZones.begin(); lit != betweenZones.end(); ++lit) {
                ...
                for (auto& tempCell : tempCells) {
                    ...
                    double const locCost = lastLoc.getLayerDistanceTo(tmpLoc);
