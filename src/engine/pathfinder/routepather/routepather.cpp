    ...
    bool RoutePather::followRoute(const Location& current, Route* route, double speed, Location& nextLocation)
    {
        ...
        if (!locationsEqual(current, currentNode)) {
            ...
            if (multiCell) {
                ...
                // new coordinates
                std::vector<ModelCoordinate> newCoords = currentNode.getLayer()->getCellGrid()->toMultiCoordinates(
                    ...
                    if (ti != nullptr) {
                        ...
                        if (cw &&
                            !cell->getLayer()->getCellGrid()->isAccessible(
                                nextLocation.getLayerCoordinates(), route->getCurrentNode().getLayerCoordinates())) {
                            if (ti->m_difflayer) {
