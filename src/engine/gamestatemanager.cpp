...
namespace FIFE {
	...
	void GameStateManager::activate(const std::string& name) {
		if( isActive(name) ) {
			Warn("GSTEST") << "Activating GameState " << name << " which is already active!";
			return;
		}
	...
	void GameStateManager::deactivate(const std::string & name) {
		...
		Log("GSTEST") << "ERROR: should not be reached!";
	}
