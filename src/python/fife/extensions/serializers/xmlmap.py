...
class XMLMapLoader:
    ...
    def parse_map(self, mapelt):
        ...
        if not _id:
            self._err("Map declared without an identifier.")
        ...
        try:
            ...
            fife.Exception
        ) as e:  # NameClash appears as general fife.Exception; any ideas?
            print(e.getMessage())
            print(
                "".join(
                    [
    ...
    def parse_imports(self, mapelt, map):
        ...
        for item in mapelt.findall("import"):
            ...
            if _file and _dir:
                loadImportFile(
                    self.obj_loader, "/".join(_dir, _file), self.engine, self.debug
                )
            elif _file:
            ...
            if self.callback:
                i += 1
                self.callback(
                    self.msg["imports"], float(i / float(len(tmplist)) * 0.25 + 0.25)
                )
    ...
    def parse_layers(self, mapelt, map):
        ...
        for layer in mapelt.findall("layer"):
            ...
            if not _id:
                self._err("<layer> declared with no id attribute.")
            if not grid_type:
                self._err("".join(["Layer ", str(_id), " has no grid_type attribute."]))
    ...
    def parse_cameras(self, mapelt, map):
        ...
        for camera in mapelt.findall("camera"):
            ...
            if not _id:
                self._err("Camera declared without an id.")
                ...
                self._err(
                    "".join(
                        [
                            "Camera ",
                            str(_id),
                            " declared without reference cell dimensions.",
                        ]
                    )
                )
            ...
            try:
                if viewport:
                    cam = map.addCamera(
                        str(_id), fife.Rect(*[int(c) for c in viewport.split(",")])
                    )
                    ...
                    cam = map.addCamera(
                        str(_id),
                        fife.Rect(
                            0, 0, screen.getScreenWidth(), screen.getScreenHeight()
                        ),
                    )
            ...
            if light_color:
                cam.setLightingColor(*[float(c) for c in light_color.split(",")])
