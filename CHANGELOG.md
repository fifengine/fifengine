...
- removed deprecated backward-compat key aliases (`Key::ENTER`, `Key::INVALID_KEY`, etc. — use module-level `fife.RETURN`, `fife.KEY_UNKNOWN` instead)
...
### Engine
  ...
  * Added option --lib-dir to scons build system to specify a shared library
    ...
    src/engine that will contain the current fife major, minor, and subminor
  ...
  * Added dependency tinyxml for reading xml files. This is stored in
    src/engine/ext/tinyxml directory. The scons scripts will attempt to use
